...
// Página /exito: el usuario vuelve aquí tras pagar en Stripe. Verificamos el
// pago contra Stripe (no nos fiamos del redirect), vaciamos el carrito y
// mostramos la confirmación. Si no está pagado, devolvemos a /carrito.
...
export function ExitoPage() {
  ...
  useEffect(() => {
    if (ranRef.current) return; // evita doble verificación en StrictMode
    ranRef.current = true;
    ...
    (async () => {
      try {
        ...
        if (res.ok && data.paid) {
          clear(); // vacía el carrito sólo si el pago está confirmado
          setState({ status: "ok", orderId: data.orderId, email: data.email, total: data.amount_total });
        } else {
          router.replace("/carrito");
        }
      } catch {
  ...
  if (state.status !== "ok") {
    return (
      <div style={{ padding: "140px 40px", textAlign: "center", color: "var(--vp-ink-muted)" }}>
        <div className="vp-eyebrow">Verificando tu pago…</div>
      </div>
    );
  }
